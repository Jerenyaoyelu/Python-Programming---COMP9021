...
def solve(cryptarithm):
    ...
    for _ in generate_possible_solutions(digits, len(letters_starting_a_word),
        ...
        equation = cryptarithm.translate(str.maketrans(dict(zip(all_letters,
...
# Permutes the last "length" members of L[: start + size],
# using "size" many indexes in L, the first of which is "start".
# Assumes that "length" is at most equal to "size".
def permute(digits, start, size, length):
    if length == 0:
        yield
        if size > 1:
            # "Simulates" the permutation of the "size" first elements
            # of L[start: ] by computing what would be the last permutation.
            if size % 2 or size == 2:
                digits[start], digits[start + size - 1] = digits[start + size - 1], digits[start]
            elif size == 4:
                digits[start: start + 3], digits[start + 3] =\
                              digits[start + 1: start + 4], digits[start]
            else:
                digits[start: start + 2], digits[start + 2: start + size - 2],\
                digits[start + size - 2], digits[start + size - 1] =\
                    digits[start + size - 3: start + size - 1],\
                    digits[start + 1: start + size - 3], digits[start + size - 1], digits[start]
    else:
        size -= 1
        length -= 1
        for i in range(size):
            ...
            if size % 2:
                digits[start + i], digits[start + size] = digits[start + size], digits[start + i]
            else:
                digits[start], digits[start + size] = digits[start + size], digits[start]
        yield from permute(digits, start, size, length)

# Generates permutations of digits where the last length_1 members of digits
# take arbitary nonzero values and the previous length_2 members of digits
# take other arbitary values (0 being possibly one of them).
def generate_possible_solutions(digits, length_1, length_2):
    ...
    # if all digits that remain have to be used for the letters not starting
    # a word, then there is no need to allocate the last one
