...
class GameOfLife(tk.Tk):
    ...
    def __init__(self):
        ...
        for i in range(10, 50, 5):
            densities.add_command(label = f'{i}%', command = lambda p = i: self.board.seed(p))
        ...
        self.next_state_button = tk.Button(buttons, text = 'Next state', command = self.board.update
                                          )
        self.next_state_button.pack(padx = 30, side = tk.LEFT)
        self.evolution_button = tk.Button(buttons, text = 'Start evolution', width = 13,
                                                                               command = self.evolve
                                         )
...
class Board(tk.Frame):
    ...
    def __init__(self):
        ...
        self.cells = [[[False] * (self.dim + 2) for _ in range(self.dim + 2)] for _ in range(2)]
        ...
        self.board = tk.Canvas(self, width = self.dim * self.cell_size + 5,
                                      height = self.dim * self.cell_size + 5, bg = self.board_colour
                              )
        self.switch = False
        self.draw_board()
        self.board.pack()
    ...
    def draw_board(self):
        for i in range(self.dim + 1):
            self.board.create_line(5, 5 + self.cell_size * i,
                                              5 + self.cell_size * self.dim, 5 + self.cell_size * i,
                                                                width = 0.5, fill = self.line_colour
                                  )
            self.board.create_line(5 + self.cell_size * i, 5, 5 + self.cell_size * i,
    ...
    def draw_cells(self):
        ...
        for i in range(self.dim):
            for j in range(self.dim):
                self.drawn_cells[i][j] = self.board.create_oval(6 + self.cell_size * i,
                                               6 + self.cell_size * j, 4 + self.cell_size * (i + 1),
    ...
    def flip_cell(self, event):
        if not self.master.evolving and 5 <= event.x <= self.cell_size * self.dim + 5 and\
                                                      5 <= event.y <= self.cell_size * self.dim + 5:
