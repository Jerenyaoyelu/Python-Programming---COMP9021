...
def evolve(i):
    ...
    number_of_neighbours[1: -1, 1: -1] = grid[: -2, : -2] + grid[: -2, 1: -1] + grid[: -2, 2:] +\
...
parser.add_argument('--pattern_size_multiplier', dest = 'pattern_size_multiplier',
                    default = 5, type = int
                   )
...
if not rle_filename.endswith('.rle'):
    print(f'Wrong file format, {rle_filename} should have .rle extension')
    sys.exit()
try:
    ...
    for line in lines:
        ...
        # run_spans will be of the form [n_0, n_1, n_2, n_3, ..., n_{2k}, n_{2k+1}] 
        # Sequences of living cells range from index n_{2i} included
        # to index n_{2i+1} excluded, i <= k.
        line = line.split('b')
        # {maybe_some_number}o...
        if line[0].find('o') >= 0:
            run_spans = [j]           
    ...
    print(f'Could not open {rle_file}, giving up.')
