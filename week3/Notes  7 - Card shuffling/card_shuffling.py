...
class CardShuffling(tk.Tk):
    ...
    def __init__(self):
        ...
        for i in range(self.deck_size):
            self.card_images[i] = tk.PhotoImage(file = 'Cards/' + str(i + 1) + '.gif')
            self.cards_to_display[i] = tk.Canvas(display, width = 74, height = 100,
                                                                 background = self.background_colour
                                                )
        tk.Button(display, text = 'Reset', command = self.reset).grid(row = 4, column = 4, pady = 40
                                                                     )
        tk.Button(display, text = 'Shuffle', command = self.shuffle).grid(row = 4, column = 8)
        tk.Label(display, text = 'Number of times the deck has been shuffled:').grid(row = 5,
                                                                          column = 5, columnspan = 4
                                                                                    )
        self.shuffles = 0
        ...
        # deck[0] and deck[1] will alternatively play the roles of a deck before and after
        #  shuffling, the latter being determined from the former.
    ...
    def reset(self):
        ...
        deck[0: self.deck_size] = range(self.deck_size)
        self.shuffles = 0
        self.nb_of_times_shuffled.set(0)
        self.display_deck()
