...
def f(for_seed, n, upper_bound):
    ...
    for e in square:
        for k in e:
            long.append(k)
    e_l=dict(Counter(long))
    if sum(e_l.values())==len(e_l):
        new_long=sorted(long)
        i=0
        while i<len(new_long):
            new_square.append(new_long[i:i+n])
            i +=n
        ordered_square=list(zip(*new_square))
    else:
        for e in e_l:
            if e_l[e]>1:
                duplicates.add(e)
