...
class TuringMachineSimulator(tk.Tk):
    ...
    def __init__(self):
        ...
        self.step_button = tk.Button(buttons, text = 'Step', command = self.step,state = tk.DISABLED
                                    )
        self.step_button.pack(padx = 30, side = tk.LEFT)
        self.continue_button = tk.Button(buttons, text = 'Continue', command = self.run_further,
                                                                                 state = tk.DISABLED
                                        )
    ...
    def tape_help(self):
        tkinter.messagebox.showinfo('Tape',
            ...
            'Control clicking to the right or to the left of the current rightmost or leftmost cell, '
            'respectively, adds a new cell.\n\n'
    ...
    def program_help(self):
        tkinter.messagebox.showinfo('Program',
            ...
            'where _state1_ and _state2_ have to be alphanumeric words with at most 8 characters, '
            '_bit1_ and _bit2_ have to be 0 or 1, and _dir_ has to be L or R.\n\n'
            'When the TM machine is in state _state1_ with its head pointing to a cell containing '
            '_bit1_, then it changes _bit1_ to _bit2_ in that cell, modifies its state to _state2_, '
            'and moves its head one cell to the right or to the left as determined by _dir_.\n\n'
            'The TM machine is supposed to be deterministic, hence the program should not contain '
            'two instructions starting with the same pair (_state1_, _bit1_).\n\n'
    ...
    def execution_help(self):
        tkinter.messagebox.showinfo('Execution',
            ...
            'it has to be pressed to restore the tape to its initial configuration, '
            ...
            'in which case the head initially points to the leftmost cell containing 1.\n\n'
            'The Step button executes one instruction, if possible; '
            'otherwise execution stops.\n\n'
            'The Continue buttom executes up to 1,000 instructions, if possible; '
            'otherwise execution stops.\n\n'
    ...
    def change_interface(self):
        {'Start': self.start,'Stop': self.stop, 'Reset': self.reset}[self.next_phase]()
    ...
    def start(self):
        ...
        for i in range(len(self.scrollable_tape.cells[0]) - 1, -1, -1):
            if self.scrollable_tape.bits[0][i] == 1:
                self.scrollable_tape.tape.itemconfig(self.scrollable_tape.cells[0][i], fill = 'red',
                                                                        font = ('normal', 0, 'bold')
                                                    )
...
class ScrollableTape(tk.Frame):
    ...
    def __init__(self):
        ...
        w = (self.nb_of_cells_without_scroll + 2 * self.cell_proportion_for_endspace) *\
                                                                                      self.cell_size
        self.tape = tk.Canvas(self, width = w, height = self.cell_size + 1, bg = self.tape_colour)
        self.draw_minimal_tape()
        self.tape.grid(row = 0)
        scrollbar = tk.Scrollbar(self, orient = tk.HORIZONTAL, command = self.tape.xview)
    ...
    # More cells can be displayed to the right (expanding the list self.cells[1])
    # or to the left (expanding the list self.cells[0]),
    # the lists self.cells[1] and self.cells[0] being handles to the textual display
    # of the bits they store, which are recorded in self.bits[1] and self.bits[0], respectively.
    # self.max_indexes[1] and self.max_indexes[0] record  the number of these extra cells,
    # respectively.
    # We use i to refer to the ith-cell on the right, and -i to refer to the ith-cell on the left.
    ...
    def determine_left_boundary(self):
        return -(self.max_indexes[0] + self.cell_proportion_for_endspace) * self.cell_size

    def determine_right_boundary(self):
        return self.cell_size * (max(self.nb_of_cells_without_scroll, self.max_indexes[1]) +
    ...
    def draw_minimal_tape(self):
        ...
        self.tape.config(scrollregion = (left_boundary, -(self.cell_size / 2), right_boundary,
                                                                                  self.cell_size + 1
                                        )
                        )
        self.tape.delete(tk.ALL)
        self.tape.create_line(-0.5 * self.cell_size, 0, -0.5 * self.cell_size, self.cell_size,
                                                                  width = 2, fill = self.cell_colour
                             )
        self.cells[0][0] = self.tape.create_text(0, self.cell_size / 2, text = self.bits[0][0])
    ...
    def draw_horizontal_lines(self, left_boundary, right_boundary):
        self.tape.create_line(left_boundary, 0, right_boundary - left_boundary, 0, width = 3,
                                                                             fill = self.cell_colour
                             )
        self.tape.create_line(left_boundary, self.cell_size, right_boundary, self.cell_size,
                                                                  width = 3, fill = self.cell_colour
                             )
    ...
    def add_cell_to_end(self, side):
        ...
        s = side * 2 - 1
        self.cells[side][i] = self.tape.create_text(i * s * self.cell_size, self.cell_size / 2,
                                                                                            text = 0
                                                   )
        self.lines[side][i] = self.tape.create_line(s * (i + 0.5) * self.cell_size, 0,
                  s * (i + 0.5) * self.cell_size, self.cell_size, width = 2, fill = self.cell_colour
                                                   )
...
class Program(tk.Frame):
    ...
    def __init__(self):
        ...
        self.source_code = tkinter.scrolledtext.ScrolledText(self, width = 23, height = 20,
                                                   ...
                                                   highlightcolor = self.program_selected_box_colour
                                                            )
...
class StateName(str):
    def check_syntactic_validity(self):
        ...
        if len(self) > 8:
            ...
            return False
        if not self.isalnum():
            tkinter.messagebox.showerror('State name error', f'"{self}" not all nonalphanumeric')
            return False
        return True
...
class Bit(str):
    def check_syntactic_validity(self):
        if self not in '01':
            tkinter.messagebox.showerror('Instruction error', f'"{self}" should be "0" or "1"')
            return False
        return True
...
class State(tk.Frame):
    def __init__(self, master):
        ...
        tk.Label(self, text = 'State: ', fg = Program.label_colour).pack(side = tk.LEFT)
...
class Iteration(tk.Frame):
    def __init__(self, master):
        tk.Frame.__init__(self, master)
        tk.Label(self, text = '  Iteration: ', fg = Program.label_colour).pack(side = tk.LEFT)
...
class Status(tk.Canvas):
    ...
    def __init__(self, master, program):
        ...
        self.status = self.create_oval(10, 10, 20, 20, fill = self.not_running_fill_colour,
