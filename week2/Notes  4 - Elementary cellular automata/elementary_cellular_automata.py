...
class ElementaryCellularAutomata(tk.Tk):
    ...
    def __init__(self):
        ...
        tk.Label(self, width = 9, height = 1, textvariable = self.shown_rule_number, fg = 'magenta'
                ).grid(row = 0, column = 1)
        self.shown_rule = tk.Canvas(self, width = 250, height = 50)
        self.shown_rule.grid(row = 0, column = 1, columnspan = 2, padx = (100, 0), pady = 10)
        tk.Button(self, text = 'Display history from single 1', command = self.work_from_single_1
                 ).grid(padx = (40, 0))
        self.shown_related_rule_numbers = tk.StringVar()
        tk.Label(self, width = 87, height = 1, textvariable = self.shown_related_rule_numbers
                ).grid(row = 1, column = 1)
        ...
        self.history = tk.Canvas(self, width = self.nb_of_steps * 4 + 3,
                                      height = self.nb_of_steps * 2 + 5, bg = self.background_colour
                                )
        self.history.grid(columnspan = 3, pady = 20)

    def input_help(self):
        tkinter.messagebox.showinfo('Input',
            'The input should be a nonnegative integer at most equal to 255, '
            'represented either in base 10 or in base 2 with exactly 8 bits.'
                                   )
    ...
    def get_rule(self):
        ...
        # (1, 1, 0) to b_1
        # (1, 1, 1) to b_0
        self.rule = {(i // 4, i // 2 % 2, i % 2): int(self.rule_bits[7 - i]) for i in range(8)}
        return True
    ...
    def display_rule_information(self):
        ...
        complementary_rule_bits = ''.join({'0': '1', '1': '0'}[c] for c in reversed(self.rule_bits))
        ...
        self.shown_related_rule_numbers.set('Mirrored rule: ' + str(mirrored_rule_number) +
               ...
               '            Mirrored complementary rule: ' + str(mirrored_complementary_rule_number)
                                           )
    ...
    def mirror(self, rule_bits):
        ...
        # (a, b, c) to R(c, b, a):
        return ''.join(self.rule_bits[i] for i in sorted(range(8),
    ...
    def work_from_single_1(self):
        ...
        # line[0] and line[1] will alternatively play the roles of a line and the following line,
        ...
        self.history.create_rectangle(2 * self.nb_of_steps + 3, 5, 2 * self.nb_of_steps + 5, 7,
        ...
        for i in range(1, self.nb_of_steps - 1):
            ...
            #           .xxxxx.
            #          .xxxxxxx.
            # Triangle increases by one cell on each side for each new line.
            # All cells beyond the boundaries of the triangle and on the same side
            # have the same state which is also computed
        ...
        line_a = line[a]
        line_not_a = line[not a]
        # For the last line, we do not compute the state beyond the boundary of the triangle
    ...
    def work_from_random_state(self):
        ...
        # - the leftmost x on the last row depends on the last . before the first x on the last row,
        # - which depends on the second last . before the first x on the second last row,
        # - which depends on the third last . before the first x on the third last row
        # ...
    ...
    def display_history_line(self, line, a, i, offset):
        ...
        for j in range(1, 2 * self.nb_of_steps):
            if line_a[j + offset]:
                self.history.create_rectangle(2 * j + 3, 2 * i + 5, 2 * j + 5, 2 * i + 7,
                                                                          fill = self.history_colour
                                             )

    def compute_cell_state(self, previous_line, line, j):
        line[j] = self.rule[previous_line[j - 1], previous_line[j], previous_line[j + 1]]
